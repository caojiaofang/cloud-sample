...
*/
@Order(0)
public class SampleConfig implements PropertySourceLocator {

	
	
	private static Log logger = LogFactory
			.getLog(SampleConfig.class);

	private RestTemplate restTemplate;
	private ConfigClientProperties defaultProperties;

	public SampleConfig(ConfigClientProperties defaultProperties) {
		...
		try {
			...
			for (String label : labels) {
				...
				if (result != null) {
					...
					HashMap<String, Object> map = new HashMap<>();
					putValue(map, "spring.datasource.username", "root");
					composite.addFirstPropertySource(
							new MapPropertySource("configClient", map));
	...
	public static String KEY_PREFIX = "json-";
	public static String NAME_SEPARATOR = ".";
	
	private Map<String, Object> doCustom(Map<String, Object> map){
		...
		try{
			for(Map.Entry<String, Object> entry : map.entrySet()){
	            ...
	            if(key.startsWith(KEY_PREFIX)){
	            	putGroupValue(myMap,key,value.toString());
	            }
	        }
			map.putAll(myMap);
	...
	private void putGroupValue(Map<String, String> map,String wholeKey,String value){
		String prefixKey = wholeKey.replace(KEY_PREFIX, "");
		ObjectMapper objectMapper = new ObjectMapper();
		Map<String, String> groupMap = new HashMap<>();
		try {
			groupMap = objectMapper.readValue(value, new TypeReference<Map<String, String>>() {});
		} catch (IOException e) {
			logger.error("json转换", e);
		}
		for(Map.Entry<String, String> entry: groupMap.entrySet()){
			map.put(prefixKey+NAME_SEPARATOR+entry.getKey(), entry.getValue());
			logger.debug("json配置参数 key ："+entry.getKey()+" 值value ："+entry.getValue());
		}
	}
}
