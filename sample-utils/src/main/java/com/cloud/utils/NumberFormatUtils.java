...
public class NumberFormatUtils {
	private final static Logger logger = LoggerFactory.getLogger(NumberFormatUtils.class);
	 ...
	 *            double
	 * @param pattern
	 *            String #:表示有数字则输出数字，没有则空，如果输出位数多于＃的位数，则超长输入 0:有数字则输出数字，没有补0 对于小数，有几个#或0，就保留几位的小数； 例如： "###.00"
	 *            -->表示输出的数值保留两位小数，不足两位的补0，多于两位的四舍五入 "###.0#" -->表示输出的数值可以保留一位或两位小数；整数显示为有一位小数，一位或两位小数的按原样显示，多于两位的四舍五入；
